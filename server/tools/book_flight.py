...
async def book_flight(wrapper: RunContextWrapper[UserInfo], input: BookFlightInput) -> BookFlightOutput:
    ...
    # STEP 1: Load selected flight option from memory context
    ...
    try:
        ...
        booking = FlightBooking(
            ...
            price_breakdown=[pb.dict() for pb in flight.price_breakdown] if flight.price_breakdown else None
        )
        session.add(booking)
        session.flush()  # To get booking.id

        # STEP 3: Save each leg
        for leg in flight.legs:
            ...
            # STEP 4: Save segments under each leg
            if leg.segments:
                for seg in leg.segments:
                    segment = FlightSegmentDB(
                        ...
                        extension_info=seg.extension_info if isinstance(seg.extension_info, list) else []
                    )
                    session.add(segment)

            # STEP 5: Save layovers under each leg
            if leg.layovers:
        ...
        # STEP 6: Store last booking metadata in memory
        ...
        # STEP 7: Prepare success message AFTER commit
        message = (
